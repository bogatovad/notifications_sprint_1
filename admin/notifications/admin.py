...
class NotificationAdmin(admin.ModelAdmin):
    ...
    def send_notification(self, request, queryset):
        for notification in queryset:
            logger.info(f'Отправка уведомления {notification} для пользователей {notification.recipients}')
            status_code = notification.send()
            if status_code == HTTPStatus.OK:
                self.message_user(request, f'{notification} успешно отправлено', messages.SUCCESS)
            else:
                self.message_user(request, f'Не удалось отправить {notification}', messages.ERROR)
